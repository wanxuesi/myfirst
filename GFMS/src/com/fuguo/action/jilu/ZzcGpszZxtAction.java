 ...
 */
public class ZzcGpszZxtAction extends BaseAction {
...
//			得到Map型的list4
		...
		for(int j=0;j<dateTitle.length;j++){
			...
			if(j<1){
				...
				sb2.append("["+dateTime+","+GPSZValue[j].intValue()+"]");
				//sb3.append("["+dateTime+","+KYZJValue[j].intValue()+"]");
				
				sb4.append("["+dateTime+","+ZZCValue[j].intValue()+"]");
			}else{
				sb.append(",["+dateTime+","+dateValue[j].intValue()+"]");
				sb2.append(",["+dateTime+","+GPSZValue[j].intValue()+"]");
				//sb3.append(",["+dateTime+","+KYZJValue[j].intValue()+"]");
