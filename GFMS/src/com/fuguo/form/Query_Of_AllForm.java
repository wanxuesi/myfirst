//Created by MyEclipse Struts
// XSL source (default): platform:/plugin/com.genuitec.eclipse.cross.easystruts.eclipse_4.1.0/xslt/JavaClass.xsl

package com.fuguo.form;

import javax.servlet.http.HttpServletRequest;

import org.apache.struts.action.ActionErrors;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionMapping;

/** 
 * MyEclipse Struts
 * Creation date: 03-19-2008
 * 
 * XDoclet definition:
 * @struts.form name="tdjhQueryForm"
 */
public class Query_Of_AllForm extends ActionForm {
	private int year;
	private int year2;
	private int start_month;//开始月份
	private int end_month;//结束月份
	private String start_rq;//日工作计划专用
	private String end_rq;//日工作计划专用
	private int weekFlag;//标志
	private String zqdm;
	private String zqmc;
	private String mmflag;
	private String jifl;
	private String sblx;//设备类型
	private String clgc;
	private String diqu;
    private String tbdw;
    private String sgdw;
    private String istd;
    private String gzcs;
    private String iskq;
    private String bdz;
    private String zy;
    private String dddw;
    private String dydj;
    private String gzjgorxl;
    private String gznr;
    private String jhkssj;
    private String jhjssj;
    private Integer ts;
    private String gzly;
    private String fxdj;
    private String scjxrq;
    private String bz;
    private String fxtjz;
    private String jxfl;
    private String zxqk;
    private String zfyy;
    private String zfbz;
    private String sjkssj;
    private String sjjssj;
    private String glrydg;
    private String bzzdg;
    private String islsjh;
    private String lstdyy;
    private String lstdpzr;
    private String tbr;
    private String jhzt;
    private String gzbz;
    private String gzfzr;
    private Integer gzrs;
    private Integer zs;
    private String flag1;
    private String flag2;
    private String flag3;
    private String flag4;
    private String flag5;
    private String flag6;
    private String flag7;
    private String flag8;
    private String flag9;
    private String flag10;
    
    private String queryDate;//查找日期
    private int zsz;//总市值
    private int ltsz;//流通市值
    
    
    private String lx;
	public String getLx() {
		return lx;
	}
	public void setLx(String lx) {
		this.lx = lx;
	}
	public String getBdz() {
		return bdz;
	}
	public void setBdz(String bdz) {
		this.bdz = bdz;
	}
	public String getBz() {
		return bz;
	}
	public void setBz(String bz) {
		this.bz = bz;
	}
	public String getBzzdg() {
		return bzzdg;
	}
	public void setBzzdg(String bzzdg) {
		this.bzzdg = bzzdg;
	}
	public String getDddw() {
		return dddw;
	}
	public void setDddw(String dddw) {
		this.dddw = dddw;
	}
	public String getDiqu() {
		return diqu;
	}
	public void setDiqu(String diqu) {
		this.diqu = diqu;
	}
	public String getDydj() {
		return dydj;
	}
	public void setDydj(String dydj) {
		this.dydj = dydj;
	}
	public int getEnd_month() {
		return end_month;
	}
	public void setEnd_month(int end_month) {
		this.end_month = end_month;
	}
	public String getEnd_rq() {
		return end_rq;
	}
	public void setEnd_rq(String end_rq) {
		this.end_rq = end_rq;
	}
	public String getFlag1() {
		return flag1;
	}
	public void setFlag1(String flag1) {
		this.flag1 = flag1;
	}
	public String getFlag10() {
		return flag10;
	}
	public void setFlag10(String flag10) {
		this.flag10 = flag10;
	}
	public String getFlag2() {
		return flag2;
	}
	public void setFlag2(String flag2) {
		this.flag2 = flag2;
	}
	public String getFlag3() {
		return flag3;
	}
	public void setFlag3(String flag3) {
		this.flag3 = flag3;
	}
	public String getFlag4() {
		return flag4;
	}
	public void setFlag4(String flag4) {
		this.flag4 = flag4;
	}
	public String getFlag5() {
		return flag5;
	}
	public void setFlag5(String flag5) {
		this.flag5 = flag5;
	}
	public String getFlag6() {
		return flag6;
	}
	public void setFlag6(String flag6) {
		this.flag6 = flag6;
	}
	public String getFlag7() {
		return flag7;
	}
	public void setFlag7(String flag7) {
		this.flag7 = flag7;
	}
	public String getFlag8() {
		return flag8;
	}
	public void setFlag8(String flag8) {
		this.flag8 = flag8;
	}
	public String getFlag9() {
		return flag9;
	}
	public void setFlag9(String flag9) {
		this.flag9 = flag9;
	}
	public String getFxdj() {
		return fxdj;
	}
	public void setFxdj(String fxdj) {
		this.fxdj = fxdj;
	}
	public String getFxtjz() {
		return fxtjz;
	}
	public void setFxtjz(String fxtjz) {
		this.fxtjz = fxtjz;
	}
	public String getGlrydg() {
		return glrydg;
	}
	public void setGlrydg(String glrydg) {
		this.glrydg = glrydg;
	}
	public String getGzbz() {
		return gzbz;
	}
	public void setGzbz(String gzbz) {
		this.gzbz = gzbz;
	}
	public String getGzcs() {
		return gzcs;
	}
	public void setGzcs(String gzcs) {
		this.gzcs = gzcs;
	}
	public String getGzfzr() {
		return gzfzr;
	}
	public void setGzfzr(String gzfzr) {
		this.gzfzr = gzfzr;
	}
	public String getGzjgorxl() {
		return gzjgorxl;
	}
	public void setGzjgorxl(String gzjgorxl) {
		this.gzjgorxl = gzjgorxl;
	}
	public String getGzly() {
		return gzly;
	}
	public void setGzly(String gzly) {
		this.gzly = gzly;
	}
	public String getGznr() {
		return gznr;
	}
	public void setGznr(String gznr) {
		this.gznr = gznr;
	}
	public Integer getGzrs() {
		return gzrs;
	}
	public void setGzrs(Integer gzrs) {
		this.gzrs = gzrs;
	}
	public String getIskq() {
		return iskq;
	}
	public void setIskq(String iskq) {
		this.iskq = iskq;
	}
	public String getIslsjh() {
		return islsjh;
	}
	public void setIslsjh(String islsjh) {
		this.islsjh = islsjh;
	}
	public String getIstd() {
		return istd;
	}
	public void setIstd(String istd) {
		this.istd = istd;
	}
	public String getJhjssj() {
		return jhjssj;
	}
	public void setJhjssj(String jhjssj) {
		this.jhjssj = jhjssj;
	}
	public String getJhkssj() {
		return jhkssj;
	}
	public void setJhkssj(String jhkssj) {
		this.jhkssj = jhkssj;
	}
	public String getJhzt() {
		return jhzt;
	}
	public void setJhzt(String jhzt) {
		this.jhzt = jhzt;
	}
	public String getJxfl() {
		return jxfl;
	}
	public void setJxfl(String jxfl) {
		this.jxfl = jxfl;
	}
	public String getLstdpzr() {
		return lstdpzr;
	}
	public void setLstdpzr(String lstdpzr) {
		this.lstdpzr = lstdpzr;
	}
	public String getLstdyy() {
		return lstdyy;
	}
	public void setLstdyy(String lstdyy) {
		this.lstdyy = lstdyy;
	}
	public String getScjxrq() {
		return scjxrq;
	}
	public void setScjxrq(String scjxrq) {
		this.scjxrq = scjxrq;
	}
	public String getSgdw() {
		return sgdw;
	}
	public void setSgdw(String sgdw) {
		this.sgdw = sgdw;
	}
	public String getSjjssj() {
		return sjjssj;
	}
	public void setSjjssj(String sjjssj) {
		this.sjjssj = sjjssj;
	}
	public String getSjkssj() {
		return sjkssj;
	}
	public void setSjkssj(String sjkssj) {
		this.sjkssj = sjkssj;
	}
	public int getStart_month() {
		return start_month;
	}
	public void setStart_month(int start_month) {
		this.start_month = start_month;
	}
	public String getStart_rq() {
		return start_rq;
	}
	public void setStart_rq(String start_rq) {
		this.start_rq = start_rq;
	}
	public String getTbdw() {
		return tbdw;
	}
	public void setTbdw(String tbdw) {
		this.tbdw = tbdw;
	}
	public String getTbr() {
		return tbr;
	}
	public void setTbr(String tbr) {
		this.tbr = tbr;
	}
	public Integer getTs() {
		return ts;
	}
	public void setTs(Integer ts) {
		this.ts = ts;
	}
	public int getYear() {
		return year;
	}
	public void setYear(int year) {
		this.year = year;
	}
	public String getZfbz() {
		return zfbz;
	}
	public void setZfbz(String zfbz) {
		this.zfbz = zfbz;
	}
	public String getZfyy() {
		return zfyy;
	}
	public void setZfyy(String zfyy) {
		this.zfyy = zfyy;
	}
	public Integer getZs() {
		return zs;
	}
	public void setZs(Integer zs) {
		this.zs = zs;
	}
	public String getZxqk() {
		return zxqk;
	}
	public void setZxqk(String zxqk) {
		this.zxqk = zxqk;
	}
	public String getZy() {
		return zy;
	}
	public void setZy(String zy) {
		this.zy = zy;
	}
	public int getWeekFlag() {
		return weekFlag;
	}
	public void setWeekFlag(int weekFlag) {
		this.weekFlag = weekFlag;
	}
	public String getSblx() {
		return sblx;
	}
	public void setSblx(String sblx) {
		this.sblx = sblx;
	}
	public String getClgc() {
		return clgc;
	}
	public void setClgc(String clgc) {
		this.clgc = clgc;
	}
	public String getJifl() {
		return jifl;
	}
	public void setJifl(String jifl) {
		this.jifl = jifl;
	}
	public String getMmflag() {
		return mmflag;
	}
	public void setMmflag(String mmflag) {
		this.mmflag = mmflag;
	}
	public String getZqdm() {
		return zqdm;
	}
	public void setZqdm(String zqdm) {
		this.zqdm = zqdm;
	}
	public String getZqmc() {
		return zqmc;
	}
	public void setZqmc(String zqmc) {
		this.zqmc = zqmc;
	}
	public int getYear2() {
		return year2;
	}
	public void setYear2(int year2) {
		this.year2 = year2;
	}
	public int getLtsz() {
		return ltsz;
	}
	public void setLtsz(int ltsz) {
		this.ltsz = ltsz;
	}
	public String getQueryDate() {
		return queryDate;
	}
	public void setQueryDate(String queryDate) {
		this.queryDate = queryDate;
	}
	public int getZsz() {
		return zsz;
	}
	public void setZsz(int zsz) {
		this.zsz = zsz;
	}
	

}

