...
9.3  2019-1-1----------------1-2
修改了hibernateUtil的底层方法sqlQuery(sql,JiluDTO.class);
增加了SQLColumnToBean.java方法，使得通过sql查询后，可以映射到相应的类上（不需要再区分大小写）。
修改相应的方法sqlQuery，同步更新到最新方法
--***需要加入Spring 的事务管理功能，能有效的防止系统产生数据不完全的操作***；

--个人配置页面需要界面美化；
--股票可以通过拼音首字母，选择。
--系统重新架构为SpringBoot + SpringMVC + Hibernate

9.22 2018-12-5------------12.31
修改了一个button的文字属性，改为盈亏/持仓量折线图
